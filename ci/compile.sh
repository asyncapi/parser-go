if [[ $TRAVIS_COMMIT_MESSAGE == *"[compile]"* ]]; then
  ...
  if [ "$TRAVIS_OS_NAME" = "windows" ]; then
    ...
    CGO_ENABLED=1 GOOS=darwin GOARCH=amd64 go build -o bin/cparser-darwin-amd64.so.dylib -buildmode=c-shared cparser/cparser.go
  elif [ "$TRAVIS_OS_NAME" = "linux" ]; then
    CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build -o bin/cparser-linux-amd64.so -buildmode=c-shared cparser/cparser.go
  fi

  git pull origin $TRAVIS_BRANCH # This is to decrease the chances of having conflicts with other compilation processes.
  git add bin
